function left_menu(g) {
    ...
    this.lists = {users: {img: 'users.png', tooltip: 'users'},
                   groups: {img: 'groups.png', tooltip: 'groups'},
                   resources: {img: 'resources.png', tooltip: 'resources'},
                   books: {img: 'books.png', tooltip: 'books'},
                   forums: {img: 'forums.png', tooltip: 'forums'},
                   glossaries: {img: 'glossaries.png', tooltip: 'glossaries'},
                   wikis: {img: 'wikis.png', tooltip: 'wikis'},
    ...
    this.init_lm_header = function() {
        ...
        for (item in this.lists) {
            ...
            if (this.gismo.get_items_number(item) > 0) {
                // list link with icon
    ...
    this.init_lm_content = function() {
        ...
        for (item in this.lists) {
            ...
            if (count > 0) {
                if (item == 'groups') {
                    // "groupings" header to toggle all at once
                    ...
                    cb_item.bind("click", {list: item}, function(event) {
                        ...
                        // toggle all users when toggling all groups
                        $('#users_list input:checkbox').prop('checked', $(this).prop('checked'));
                    ...
                    for (var grouping in this.gismo.static_data[item]) {
                        ...
                        cb_label = $("<label></label>").html("<b>" + lab.toUpperCase() + ' <span style="white-space:nowrap;">(' + groups_count + " GROUPS)</span></b>");
                        cb_label.addClass("cb_label");
                        cb_label.prepend(cb_item);
                    ...
                    cb_item.bind("click", {list: item}, function(event) {
                        $('#' + event.data.list + '_list input:checkbox').prop('checked', $(this).prop('checked'));
                        if(event.data.list=='users') { // toggle groups when toggling all users
                            $('#groups_list input:checkbox').prop('checked', $(this).prop('checked'));
                        }
    ...
    this.get_selected_items = function () {
        ...
        for (var item in this.lists) {
            ...
            $("#" + this.get_list_container_id(item) + " input:checkbox:checked").each(function (index) {
                selected_items[item].push(parseInt($(this).val()));            
            });    
        }
    ...
    this.set_menu = function (fresh) {
        // enabled menus (def)
        var all = ['users', 'groups', 'resources', 'books', 'forums', 'glossaries', 'wikis', 'assignments', 'quizzes'];
        var enabled = ['users', 'groups', 'resources', 'books', 'forums', 'glossaries', 'wikis', 'assignments', 'quizzes'];
        var list = enabled[0];
