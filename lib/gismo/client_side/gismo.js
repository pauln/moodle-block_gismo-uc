function gismo(config, srv_data, static_data, course_start_time, current_time) {
    ...
    this.menu = { 'student-resources-access': ['users', 'groups', 'resources'],
                  'student-resources-access:users-details': ['resources'],
                  'student-books-access': ['users', 'groups', 'books'],
                  'student-books-access:users-details': ['books'],
                  'student-forums-access': ['users', 'groups', 'forums'],
                  'student-forums-access:users-details': ['forums'],
                  'student-glossaries-access': ['users', 'groups', 'glossaries'],
                  'student-glossaries-access:users-details': ['glossaries'],
                  'student-wikis-access': ['users', 'groups', 'wikis'],
                  'student-wikis-access:users-details': ['wikis'],
                  'student-accesses': ['users', 'groups'],
                  'student-accesses-overview': ['users', 'groups'],
                  'resources-students-overview': ['users', 'groups', 'resources'],
                  'resources-access': ['users', 'groups', 'resources'],
                  'resources-access:resources-details': ['users', 'groups'],
                  'books-students-overview': ['users', 'groups', 'books'],
                  'books-access': ['users', 'groups', 'books'],
                  'forums-students-overview': ['users', 'groups', 'forums'],
                  'forums-access': ['users', 'groups', 'forums'],
                  'glossaries-students-overview': ['users', 'groups', 'glossaries'],
                  'glossaries-access': ['users', 'groups', 'glossaries'],
                  'wikis-students-overview': ['users', 'groups', 'wikis'],
                  'wikis-access': ['users', 'groups', 'wikis'],
                  'activitysummary-students-overview': ['users', 'groups', 'forums', 'glossaries', 'wikis'],
                  'activitysummary-access': ['users', 'groups', 'forums', 'glossaries', 'wikis'],
                  'assignments': ['users', 'groups', 'assignments'], 
                  'quizzes': ['users', 'groups', 'quizzes']};
    this.menu_default = {   'student-resources-access': 0,
                            ...
                            'resources-students-overview': 2,
                            'resources-access': 2,
                            'resources-access:resources-details': 0,
                            'books-students-overview': 2,
                            'books-access': 2,
                            'books-access:books-details': 0,
                            'forums-students-overview': 2,
                            'forums-access': 2,
                            'forums-access:forums-details': 0,
                            'glossaries-students-overview': 2,
                            'glossaries-access': 2,
                            'glossaries-access:glossaries-details': 0,
                            'wikis-students-overview': 2,
                            'wikis-access': 2,
        ...
        switch (this.get_full_type()) {
            case 'student-accesses':
                if (this.static_data["users"].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    // yticks
                    count = selected_items["users"].length;
                    for (item in this.static_data["users"]) {
                        if (jQuery.inArray(this.static_data["users"][item].id, selected_items["users"]) != -1) {
                    ...
                    for (item in this.current_analysis.data) {                        
                        if (jQuery.inArray(this.current_analysis.data[item].userid_log, selected_items["users"]) != -1) {
                ...
                if (this.static_data["users"].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    for (item in this.static_data["users"]) {
                        if (jQuery.inArray(this.static_data["users"][item].id, selected_items["users"]) != -1) {
                            ...
                            yticks.push(this.static_data["users"][item].id);
                        }
                    }
            ...
            case 'student-wikis-access:users-details':
                ...
                if (this.static_data[this_type].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    for (item in this.static_data[this_type]) {
                        if (jQuery.inArray(this.static_data[this_type][item].id, selected_items[this_type]) != -1) {
            ...
            case 'activitysummary-students-overview':
                ...
                if (this.static_data["users"].length > 0 && this.static_data[this_type].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    for (item in this.static_data[this_type]) {
                        ...
                        if (jQuery.inArray((this.static_data[this_type][item].id), selected_items[real_type]) != -1) {
                    ...
                    for (item in this.current_analysis.data) {
                        ...
                        key = userid + "_" + resid;
                        if (jQuery.inArray(userid, selected_items["users"]) != -1 &&
                            jQuery.inArray(resid, selected_items[real_type]) != -1) {
                ...
                if (this.static_data[this_type].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    for (item in this.static_data[this_type]) {
                        ...
                        if (jQuery.inArray(this.static_data[this_type][item].id, selected_items[real_type]) != -1) {
                            lines.push(0);
                            xticks.push(this.util.intelligent_substring(this.static_data[this_type][item].name, true));
                            yticks.push(this.static_data[this_type][item].id);
                        }
                    }
            ...
            case 'assignments':
                if (this.static_data["users"].length > 0 && this.static_data["assignments"].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    count = selected_items["assignments"].length;
                    for (item in this.static_data["assignments"]) {
                        if (jQuery.inArray(this.static_data["assignments"][item].id, selected_items["assignments"]) != -1) {
                    ...
                    colors = this.get_series_colors();
                    for (item in this.current_analysis.data) {                        
                        if (jQuery.inArray(this.current_analysis.data[item].userid, selected_items["users"]) != -1 &&
                            jQuery.inArray(this.current_analysis.data[item].test_id, selected_items["assignments"]) != -1) {
            ...
            case 'quizzes':
                if (this.static_data["users"].length > 0 && this.static_data["quizzes"].length > 0 && this.util.get_assoc_array_length(this.current_analysis.data) > 0) {
                    ...
                    for (item in this.static_data["quizzes"]) {
                        if (jQuery.inArray(this.static_data["quizzes"][item].id, selected_items["quizzes"]) != -1) {
                    ...
                    for (item in this.current_analysis.data) {                        
                        if (jQuery.inArray(this.current_analysis.data[item].userid, selected_items["users"]) != -1 &&
                            jQuery.inArray(this.current_analysis.data[item].test_id, selected_items["quizzes"]) != -1) {
        ...
        if (data == undefined || data == null || !(this.util.get_assoc_array_length(data) > 0)) {
            ...
            switch (this.get_full_type()) {
                ...
                case 'quizzes':
                    ...
                    this.current_analysis.plot = $.jqplot(this.plot_id, data.lines, {
                        seriesDefaults: { neighborThreshold: 0, pointLabels: { show: false }, showLine: false, markerOptions:{ size: msize, shadow: false }/*, gridPadding: {top:5, right:5, bottom:5, left:5}*/ },
        ...
        if (this.static_data[item] != undefined && this.static_data[item] != null) {
            if (item == 'groups') {
                for (var k in this.static_data[item]) {
                    for (var j in this.static_data[item][k].groups)
                        count++;
                }
            } else {
